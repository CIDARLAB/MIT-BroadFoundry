...
Engineering disciplines rely on the use of standards to clearly communicate design information and allow for the construction of large and complex systems. As synthetic biology 
...
\caption{\label{fig:overview}Overview of the core dnaplotlib functionality. (\textbf{A}) The full set of SBOL Visual parts is available in both forward and reverse orientations. (\textbf{B}) The size, color and shape of all elements be easily customised by the user. (\textbf{C}) To allow for direct comparison between a genetic design and nucleotide-level data such as RNA-seq read depths, trace-based renderers are provided. These use indicators of actual part widths on the design backbone (CDS length and small filled rectangles) to enable a visual alignment of design information with trace data. (\textbf{D}) An example rendering of a library of genetic design variants implementing the same 3-input (black promoters), 1-output (black CDS) device. Colors have been used to link repressor genes and their cognate promoters.}
\end{figure}
