            ...
            for comparingLevelGates in range(levelGates+1):
                ...
                if comparingLevel==currentLevel:
                    for betaCircuitIndex in range(comparingLevel.index(alphaCircuit)+1,len(comparingLevel)):
                        ...
                        newCircuit = "("+alphaCircuit+"."+betaCircuit+")"
                        numGates = gateCounter(newCircuit)
                        if ((newCircuit in ignore) or numGates>num or (numGates>1 and(containsEqualSubCircuit(newCircuit) or (getTruthValue(newCircuit) in notAllowed)))):
                            continue
...
def wrapWithUnusedFromStock(startNumGates,endNumGates,directory):
    ...
    if directory[-1] != "/":
        directory += "/"
    startFileName = "C:/Users/Arinze/SkyDrive/UROP_Summer_2015/GateMinimization2/Stock/"+str(startNumGates)+"gates.txt"
    for i in range(startNumGates+1):
        unused = unused + checkForUse(i,unused,startFileName)
    for currentGates in range(startNumGates+1,endNumGates+1):
        if currentGates < endNumGates:
...
def minCircuitFinder3(num):
    #immediately removes circuits not used as it goes
    allLevels = {}
    currentLevel = ['a','b','c','0']
    allLevels[0] = currentLevel
    foundCircuits = {}
    ...
    while not(levelGates>=num+1 or len(foundCircuits)>=256):
        ...
        for alphaCircuit in currentLevel:
            ...
            if not(alphaTruthValue in foundCircuits):
                ...
                if alphaTruthValue in foundTruths:
                    foundCircuits[alphaTruthValue].append(alphaCircuit)
                elif alphaTruthValue not in foundTruths and levelGates>0:
                    currentLevel.remove(alphaCircuit)
        if len(foundCircuits)>=256:
            return foundCircuits,allLevels
        if levelGates == num:
            endTime = time.time()
