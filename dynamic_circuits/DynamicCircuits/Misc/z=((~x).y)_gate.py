...
mBx = 30    # induced rate of transcription (30 transcript/min) by IPTG
amy = np.log(2)/My_HALFLIFE  # degradation rate of mRNA (half-life = 2 min)
mBy1 = 30    # induced rate of transcription (30 transcript/min)
mBy2 = 30    # induced rate of transcription (30 transcript/min)

nx = 2                   # hill coefficient for protein IPTG
Kmx = 40                 # repression threshold of promoter for IPTG, monomers per cell
ny1 = 2                   # hill coefficient
Kmy1 = 30000                 # repression threshold of promoters, monomers per cell
ny2 = 2                   # hill coefficient
Kmy2 = 40                 # repression threshold of promoters, monomers per cell
...
def f(state, t, var1, var2):
        ...
        try:
            ...
            dmY_dt = -amy*mY + mBy1*(Kmy1**ny1)/(Xi**ny1+Kmy1**ny1) + mBy2*(Kmy2**ny2)/(IPTGi2**ny2+Kmy2**ny2)
        except OverflowError:
            dmX_dt = -amx*mX + mBx/(1+(IPTGi/Kmx)**nx)
            dmY_dt = -amy*mY + mBy1/(1+(Xi/Kmy1)**ny1) + mBy2/(1+(IPTGi2/Kmy2)**ny2)
        #protein production
        dX_dt = Bx*mX - ax*Xi
        dY_dt = By*mY - ay*Yi
...
soln_f = odeint(f, initc, t, args=(var1,var2))
