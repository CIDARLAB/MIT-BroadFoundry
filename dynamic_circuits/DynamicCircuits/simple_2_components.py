...
#Input
parts = {'p1':['promoter', 0, 5, 0.1, 0, 0.5, 2, 'p2', 'rep', 'p2', 'rep'], 
         'p2':['promoter', 0, 4, 0.1, 0, 0.5, 2, 'p1', 'rep', 'p1', 'rep']}
...
def f(init, t):
    ...
    for p in parts:
        if iTyp[i] == 'rep': # Repressor Equation
            X = parts[inp[i]][1]
            der[i] = B[i]/(1+np.power(X/K[i],n[i])) - a[i]*Xi
        elif iTyp[i] == 'act': # Activator Equation
            X = parts[inp[i]][1]
            der[i] = B[i]*(np.power(X,n[i])/(np.power(X,n)+np.power(K[i],n[i]))) - a[i]*Yi           
        i+=1
    print der
    return der
