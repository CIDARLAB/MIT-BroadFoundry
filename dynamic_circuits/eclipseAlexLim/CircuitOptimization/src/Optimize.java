				...
				while (true){
					...
					if (betaCost != alphaCost){
						for (Circuit beta : betaCircuits){
							//Print an update approximately every 5 minutes
							if (((System.currentTimeMillis() - currTime)/1000)>=100.0){
								currTime = System.currentTimeMillis();
								double timeSoFar = (currTime - startTime)/1000;
								double prog = 100*Integer.toUnsignedLong(indexAlpha*betaCircuits.size() + betaCircuits.indexOf(beta))/Integer.toUnsignedLong(alphaCircuits.size()*betaCircuits.size());
								System.out.println(prog+ "% complete with making circuits by combining " + alphaCost + " with "+betaCost+ " circuits after "+timeSoFar+"seconds");
							}
							for (String operator:symmetricOps){
					...
					// want to stop when we reach the alpha circuit.
						...
						while (true){
							...
							if (((System.currentTimeMillis() - currTime)/1000)>=100.0){
								currTime = System.currentTimeMillis();
								double timeSoFar = (currTime - startTime)/1000;
								double prog = 100.0*Integer.toUnsignedLong(indexAlpha*betaCircuits.size() + indexBeta)/Integer.toUnsignedLong(alphaCircuits.size()*betaCircuits.size());
								System.out.println(prog+ "% complete with making circuits by combining " + alphaCost + " with "+betaCost+ " circuits after "+timeSoFar+"seconds");
							}
			...
			while (true){
				...
				while(betaCost<=alphaCost){
					...
					if (betaCost != alphaCost){
						for (Circuit beta : betaCircuits){
							//Print an update approximately every 5 minutes
							if (((System.currentTimeMillis() - currTime)/1000)>=300.0){
								currTime = System.currentTimeMillis();
								double timeSoFar = (currTime - startTime)/1000;
								double prog = 100*Integer.toUnsignedLong(indexAlpha*betaCircuits.size() + betaCircuits.indexOf(beta))/Integer.toUnsignedLong(alphaCircuits.size()*betaCircuits.size());
								System.out.println(prog+ "% complete with making circuits by combining " + alphaCost + " with "+betaCost+ " circuits after "+timeSoFar+"seconds");
