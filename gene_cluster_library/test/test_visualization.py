...
plt.tight_layout()
fig.savefig('./visualizations/variant_rendering.pdf', transparent=True)
...
plt.tight_layout()
fig.savefig('./visualizations/library_rendering.pdf')
...
plt.tight_layout()
fig.savefig('./visualizations/arch_and_trace.pdf')
...
gcv.plot_traces_with_arch(ax_arch, ax_traces, nifs, '75', [phys_reads['75'], phys_reads['80']], start_idx=1, 
	                      end_idx=-2, linewidth=1.2, scaleadjust=0.7)
plt.tight_layout()
fig.savefig('./visualizations/arch_and_multiple_traces.pdf')
...
for v in nifs.variants.keys():
	...
	# Load the traces for predicted and measured
	phys_reads = gca.load_stata_strand_data('./data/phys_depths3.csv')
	gcv.plot_traces_with_arch(ax_arch, [ax_traces], nifs, v, [phys_reads[v]], start_idx=1, 
		                      end_idx=-2, linewidth=1.2, scaleadjust=0.7)
	plt.tight_layout()
