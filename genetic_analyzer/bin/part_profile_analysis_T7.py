...
def main():
    ...
    for s in settings.keys():
        if s != 'None':
            samples.append(s)
            chr_promoters = ga.characterize_promoters(settings, s, upstream_bp=15, downstream_bp=100)
            ga.save_characterization_data(settings, s, chr_promoters, part_type='promoter')
            chr_terminators = ga.characterize_terminators(settings, s, upstream_bp=15, downstream_bp=15)
            ga.save_characterization_data(settings, s, chr_terminators, part_type='terminator')
            chr_ribozymes = ga.characterize_ribozymes(settings, s, upstream_bp=15, downstream_bp=15)
