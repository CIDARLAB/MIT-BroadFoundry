...
# Load the data
p_perf_data = load_perf_data(RESULTS_PREFIX+'promoter_rsem_pref.txt')
t_perf_data = load_perf_data(RESULTS_PREFIX+'terminator_rsem_pref.txt')
...
for promoter in sorted(p_perf_data.keys()):
	...
	# Plot the data
	ax = plt.subplot(1,len(p_perf_data.keys()),ax_idx)
...
for promoter in sorted(p_perf_data.keys()):
	...
	ax.set_xlabel('Tx IN (TPM)')
	ax.set_ylabel('Strength (TPM)')
	ax_idx += 1
plt.tight_layout()
...
for terminator in sorted(t_perf_data.keys()):
	...
	# Plot the data
	ax = plt.subplot(1,len(t_perf_data.keys()),ax_idx)
...
for terminator in sorted(t_perf_data.keys()):
	# Extract the data
	t_tx_in = np.array([x[2][0] for x in t_perf_data[terminator]])/10000.0
	t_tx_out = np.array([x[-1] for x in t_perf_data[terminator]])
	# Plot the data
	ax = plt.subplot(1,len(t_perf_data.keys()),ax_idx)
	ax.scatter(t_tx_in, t_tx_out)
	ax.set_title(terminator)
	ax.set_xlabel('Tx IN (x10000 TPM)')
	ax.set_ylabel('Strength (% TPM)')
	ax.set_ylim([-0.1,1.1])
	ax_idx += 1
	print terminator, np.max(t_tx_in)
plt.tight_layout()
plt.savefig('./rsem_stata_terminator_tx_in_out.pdf', transparent=True)
plt.close('all')
